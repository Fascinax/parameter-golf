...
QUANT_MAX = 31  # Int6: [-31, 31] for better compression

_QAT_ENABLED = False

def fake_quantize_int8(w: Tensor) -> Tensor:
    if w.ndim != 2 or w.numel() <= INT8_KEEP_FLOAT_MAX_NUMEL:
        return w
    w32 = w.float()
    clip_abs = torch.quantile(w32.abs(), INT8_CLIP_Q, dim=1)
    scale = (clip_abs / QUANT_MAX).clamp_min(1.0 / QUANT_MAX)
    q = torch.clamp(torch.round(w32 / scale[:, None]), -QUANT_MAX, QUANT_MAX)
...
class CastedLinear(nn.Linear):
    def forward(self, x: Tensor) -> Tensor:
        ...
        if _QAT_ENABLED:
            w = w + (fake_quantize_int8(w) - w).detach()
...
class GPT(nn.Module):
    ...
    def _get_block(self, effective_idx: int) -> Block:
        return self.blocks[effective_idx % len(self.blocks)]
